...
if ($connectionOk && $connection !== false) {
    ...
    $expectedRolePermissionCount = 162;
    $rolePermissionCount = erp_dash_int(erp_dash_scalar($connection, 'SELECT COUNT(*) FROM dbo.core_role_permissions'));
    erp_dash_add_check(
        $checks,
        'D11',
        'role_permissions',
        (string)$expectedRolePermissionCount,
        (string)$rolePermissionCount,
        $rolePermissionCount === $expectedRolePermissionCount
    );
