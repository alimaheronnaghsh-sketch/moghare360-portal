...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && isset($_POST['complete_task'])) {
    ...
    $res = work360_complete_task($conn, $user, $id, trim((string)($_POST['completion_note'] ?? '')));
    $ok = !empty($res['ok']);
...
$doneMode = strtoupper((string)$task['status_code']) === 'DONE' ? work360_detect_completion_mode_from_history($conn, $id) : null;
work360_layout_start('جزئیات کار', 'task-view.php');
work360_flash($msg, $ok);
?>
<div class="m360-card">
  <h3><?= work360_h((string)$task['title']) ?> <span class="m360-badge"><?= work360_h((string)$task['task_code']) ?></span>
  <?php if (strtoupper((string)$task['status_code']) === 'DONE'): ?>
    <span class="m360-badge m360-badge-ok">بسته‌شده</span>
    <?php if ($doneMode && $doneMode !== 'SELF'): ?>
      <span class="m360-badge m360-badge-warn"><?= work360_h(work360_completion_badge_fa($doneMode)) ?></span>
    <?php endif; ?>
  <?php endif; ?>
  </h3>
  ...
  <?php if (!empty($task['completion_note'])): ?><p>یادداشت اتمام: <?= work360_h((string)$task['completion_note']) ?></p><?php endif; ?>
  <p><?= nl2br(work360_h((string)($task['description'] ?? ''))) ?></p>
  <div class="w360-actions">
    <?php if (!empty($auth['ok'])): ?>
    <form method="post" style="display:inline-flex;gap:.4rem;align-items:center;flex-wrap:wrap">
      <?= work360_csrf_field() ?>
      <input type="hidden" name="complete_task" value="1">
      <?php if (!empty($auth['note_required'])): ?>
      <input type="text" name="completion_note" placeholder="یادداشت اتمام" required style="min-width:180px">
      <?php endif; ?>
      <button type="submit" class="m360-btn m360-btn-primary"><?= work360_h((string)$auth['label_fa']) ?></button>
    </form>
    <?php endif; ?>
