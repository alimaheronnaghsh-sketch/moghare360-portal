...
$where = $scope['sql'];
$params = $scope['params'];
// STAFF always own tasks only — scope already enforces assigned_to
...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
    ...
    if ($task && work360_can_access_task($user, $task)) {
        $map = ['start'=>'IN_PROGRESS','waiting'=>'WAITING','blocked'=>'BLOCKED','done'=>'DONE'];
        if (isset($map[$action])) {
...
<tr>
  ...
  <td>
    ...
    <form method="post" style="display:inline"><?= work360_csrf_field() ?><input type="hidden" name="task_id" value="<?= (int)$r['task_id'] ?>">
      ...
      <button name="action" value="done" class="m360-btn m360-btn-primary" style="margin:0;padding:.3rem .5rem;font-size:.75rem">انجام</button>
    </form>
    <?php endif; ?>
  </td>
</tr>
