...
$sqlDestDir = Join-Path $PackageDir "sql"
New-Item -ItemType Directory -Path $sqlDestDir -Force | Out-Null
Copy-Item (Join-Path $RepoRoot "public_html\sql\sqlserver\v1_saas_activation_foundation.sql") (Join-Path $sqlDestDir "v1_saas_activation_foundation.sql") -Force
