 ...
 * No database. No local business persistence.
 */
...
{
    return (string)(mirror_config()['BRAND_NAME'] ?? 'MOGHAREH MOTORS / MOGHARE360');
}
...
function mirror_render_head(string $title, string $activeNav = ''): void
{
    ...
    echo '<p class="m360-mirror-banner">رابط آینه moghareh360.ir — بدون دیتابیس هاست · بدون ذخیره داده</p></div></div>';
    echo '<nav class="m360-nav">';
    $links = [
        'index' => ['/', 'خانه'],
        'customer' => ['customer-request.php', 'مشتری'],
        'staff' => ['staff-login.php', 'پرسنل'],
        'owner' => ['owner-login.php', 'مالک'],
        'company' => ['company-owner-dashboard.php', 'مالک کمپانی'],
    ];
...
function mirror_render_foot(): void
{
    ...
    echo 'Mirror Interface Only — No Host Database — No Cloud Storage — Data on Master Laptop Server only';
    echo '<div class="m360-install-hint">برای نصب PWA: از منوی مرورگر «افزودن به صفحه اصلی» را انتخاب کنید.</div>';
    echo '</footer></div>';
