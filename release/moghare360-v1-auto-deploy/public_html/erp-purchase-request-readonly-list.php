...
/**
 * MOGHARE360 ERP Purchase Request Read-Only List
 *
...
const ERP_M26_LIST_ACTION = 'purchase.request.list';
...
function erp_m26_list_require_first_existing(array $candidatePaths, string $label): void
{
...
function erp_m26_list_helper_candidates(string $fileName): array
{
    return [
 ...
 */
function erp_m26_list_fetch_rows($connection, string $sql, array $params = []): array
{
...
function erp_m26_list_h(string $value): string
{
    return htmlspecialchars($value, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
}

function erp_m26_list_display(string $value): string
{
    return erp_m26_list_h(trim($value) === '' ? '—' : $value);
}

/**
 * @return array<string, mixed>
 */
function erp_m26_list_guard_eval($connection, int $userId, string $actionKey): array
{
...
try {
    erp_m26_list_require_first_existing(erp_m26_list_helper_candidates('erp-auth-context.php'), 'erp-auth-context.php');
    erp_m26_list_require_first_existing(erp_m26_list_helper_candidates('erp-permission-guard.php'), 'erp-permission-guard.php');
} catch (Throwable $exception) {
    echo '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>List Error</title></head><body>';
    echo '<p>ERP Purchase Request list could not be loaded.</p></body></html>';
    exit(1);
}
...
try {
    ...
    $guardList = erp_m26_list_guard_eval($connection, $userId, ERP_M26_LIST_ACTION);
    ...
    $listRows = erp_m26_list_fetch_rows(
        $connection,
        'SELECT TOP 50
            purchase_request_id,
            jobcard_id,
    ...
    $errorMessage = 'ERP Purchase Request list could not be completed.';
} finally {
    if ($connection !== false) {
...
<head>
    ...
    <title>Mission 26 - Purchase Request List</title>
    <style>
...
<body>
<div class="wrap">
    <div class="banner">LOCAL READ-ONLY PURCHASE REQUEST LIST - NO WRITE</div>

    <div class="card">
        <h1>Purchase Request Read-Only List</h1>
        <p>Mission 26 - SELECT only - No form - No write</p>
        <?php if ($errorMessage !== ''): ?>
            <p class="fail"><?= erp_m26_list_h($errorMessage) ?></p>
        <?php endif; ?>
        <p>
    ...
    <div class="card">
        ...
        <table class="summary-table">
            <tbody>
                <tr><th>PHP version</th><td><?= erp_m26_list_h($phpVersion) ?></td></tr>
                <tr><th>ODBC extension</th><td><?= $odbcAvailable ? 'Available' : 'Missing' ?></td></tr>
                <tr><th>Connection</th><td><?= erp_m26_list_h($connectionStatus) ?> — <?= erp_m26_list_h($connectionDetail) ?></td></tr>
                <tr><th>user_id</th><td><?= erp_m26_list_h((string)$userId) ?></td></tr>
                <tr><th>username</th><td><?= erp_m26_list_h($username) ?></td></tr>
                <tr><th>guard purchase.request.list</th><td><?= erp_m26_list_h($guardListLabel) ?></td></tr>
    ...
    <div class="card">
        ...
        <?php if ($listRows === []): ?>
            <p>No Purchase Request rows found.</p>
        <?php else: ?>
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    <?php foreach ($listRows as $row): ?>
                        <?php $detailId = (int)($row['purchase_request_id'] ?? 0); ?>
                        <tr>
                            <td><?= erp_m26_list_display($row['purchase_request_id'] ?? '') ?></td>
                            <td><?= erp_m26_list_display($row['jobcard_id'] ?? '') ?></td>
                            <td><?= erp_m26_list_display($row['service_operation_id'] ?? '') ?></td>
                            <td><?= erp_m26_list_display($row['part_id'] ?? '') ?></td>
                            <td><?= erp_m26_list_display($row['requested_part_name'] ?? '') ?></td>
                            <td><?= erp_m26_list_display($row['requested_quantity'] ?? '') ?></td>
                            <td><?= erp_m26_list_display($row['request_status'] ?? '') ?></td>
                            <td><?= erp_m26_list_display($row['requested_at'] ?? '') ?></td>
                            <td><a href="erp-purchase-request-detail.php?purchase_request_id=<?= erp_m26_list_h((string)$detailId) ?>">Open</a></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
