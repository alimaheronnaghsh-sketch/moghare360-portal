...
function mirror_zip_has(string $zipPath, string $needle): bool
{
    ...
    for ($i = 0; $i < $zip->numFiles; $i++) {
        ...
        if (str_contains($name, $needleNorm)) {
            $zip->close();
            return true;
        }
    }
...
foreach ($required as $item) {
    $results[] = [
        ...
        'pass' => mirror_zip_has($zipRepo, $item),
    ];
}
...
$lintCode = 1;
if (is_file($client)) {
    $phpExe = is_file('C:\\xampp\\php\\php.exe') ? 'C:\\xampp\\php\\php.exe' : 'php';
    $lintOut = (string)shell_exec(escapeshellarg($phpExe) . ' -l ' . escapeshellarg($client) . ' 2>&1');
    $lintCode = str_contains($lintOut, 'No syntax errors') ? 0 : 1;
}
$results[] = [
    'name' => 'mirror-api-client.php syntax',
    'pass' => is_file($client) && $lintCode === 0,
];
